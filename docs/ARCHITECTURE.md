...
One switchboard per machine. MVP: one machine. Multi-box channels (M2+) follow the model in
§Multi-box below — one *home* switchboard per channel, members resident wherever their session runs.
...
- **API** — one WebSocket (subscribe/post/act) + small REST (pairing, history pages, blob
  fetch). Same API for all three surfaces and for the `codor` CLI.
...
(`session_ref`), and the switchboard, your terminal, and the channel are all merely clients of it.
...
- **Mirrored — joining from a terminal**: `/codor join` from inside a *live* TUI session
  ...
  the channel (Claude Code: hooks; Codex: `notify` config + session-file tailing), and inbound
  deliveries **queue** with a nudge shown in the channel ("mirrored — deliveries wait for the
  operator's terminal"). When the TUI exits, the switchboard **adopts** the session via
  `attach()` and the queue drains.
- **Jumping into a member — the reverse**: `codor attach <member>` (or a "jump in" button in
  ...
  `codex resume <rollout-id>` — in your terminal. You're now driving the *same session the channel
...
Same member, same context, custody moving freely between channel and terminal — that's why
...
- **Apple Watch** (SwiftUI, started from claude-watch's design): three screens — inbox
  (messages addressed to you, asks, approvals, budget holds), channel glance (who's running, spend
...
- **CLI** (`codor`): join/spawn/attach/list/post/tail. Also the thing the Codex-side skill
  shells out to.

### The `/codor` skill

Claude Code skill (and an AGENTS.md snippet for Codex) so a live session can self-register:
`/codor join traderjoe-eng --as planner`. The skill just calls the local CLI, which talks to
the unix socket — no HTTP, no tokens on disk beyond the socket's filesystem permissions.

### The ledger

Per-channel shared memory (PROTOCOL §6): an Obsidian-compatible markdown vault under
`~/.codor/rooms/<room>/ledger/`, bootstrapped by the switchboard (folders + note templates)
when enabled. The switchboard watches it (fs events), posts change notices to the channel, resolves
...
### Multi-box: channel home vs. member residency

The two concepts multi-machine setups must not conflate:

- **Every channel has exactly one home switchboard.** The home assigns message ids, stores the
  history and run blobs, hosts the ledger vault, and runs the router. If the home box is off,
  the channel is down — deliberate, the same deal as any self-hosted service. Put a channel's home on
  the box that's always on (the desk box), not the laptop.
- **Members reside wherever their session runs** (`Member.host`). A channel homed on the desk can
...
- **The ledger lives on the home box only.** No shared filesystem, no sync conflicts — remote
  ...
  `codor ledger …`, which routes to the home switchboard; `codor ledger pull` can drop a
...
So: a channel/workspace is *anchored to* one machine but not *confined to* it. If you want fully
independent workspaces per machine, simply home different channels on different boxes.

### Bridges (Slack, Telegram) — optional, eyes open

Partyline surfaces parties in Slack; Codor supports the same as an explicit **bridge**: a
small process that is just another API client, pairing a channel with an external channel. It
mirrors channel traffic outward and posts inbound platform messages as bridge-relayed human
...
community territory. The privacy contract is explicit: **bridging a channel knowingly exports that
channel's content to the platform's servers** — enabling it is an owner/admin action, the channel
...
Multi-human channels carry partyline-style org access control — without accounts. An **org** is a
...
and channels. Agent members don't have roles — what an agent may *do on the machine* is its harness
policy chip; roles govern what *humans* may do to the channel. Schema ships in M0 (`Member.role`),
enforcement + invite flows land with multi-human channels in M5.
...
| Blind ciphertext relay | partyline-sh relay + `ptysess` (Noise NNpsk0, key in URL fragment) | pattern / candidate depend | same philosophy as our push relay; M0 audit checks how separable it is from their hosted control plane — if it's a clean generic pipe, run it wholesale for channel sync |
...
- Surface offline → clients are caches; on reconnect they page missed history via REST. Push
  relay down → notifications degrade, channel state unaffected.
- Clock: message ids are the ordering truth (per-channel monotonic, assigned by the single owning
  switchboard); timestamps are display-only. Cross-host channels order by (host lamport, id) —
